...
4. **M** · **Multi-factor authentication (MFA)** — a second verification step for secure access.
...
*Later:* Key Travel Agent cross-org support & act-on-behalf (audited) · self-service organisation onboarding (an org adds/configures its own profile) · automatic HR-system sync (SCIM/HRIS).
...
*Later:* parallel / conditional / more-than-3-level chains.
...
*Later:* individual traveller payments.
...
*Later:* reporting dashboards & charts · traveller safety tracking (duty-of-care) · **native mobile app (iOS/Android)**.
